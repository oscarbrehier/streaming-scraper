...
async function handleNas(media) {
    try {
        ...
        if (!data.ok) {
            ...
            return new ErrorObject("Failed to fetch nas", "xprime - nas", data.status, "Check if the URL is valid or the server is accessible.", true, true);
        }

        data = await data.json();
        if (data.status !== "ok") {
            return new ErrorObject("Nas reported: " + data.details, "xprime - nas", 404, undefined, true, false);
        }

        return handleSuccess(data, "en");
    } catch (error) {
        console.error(`Error during fetch: ${error.message}`);
        return new ErrorObject(`Unexpected error: ${error.message}`, "xprime - nas", 500, "An unexpected error occurred while fetching nas.", true, true);
    }
}

async function handlePrimebox(media) {
    let data = await fetch(DOMAIN + `primebox?name=${media.title}&year=${media.releaseYear}`, {
        "headers": {
            Accept: "*/*",
            Referer: DOMAIN + "watch/" + media.tmdb,
            Origin: DOMAIN
    ...
    if (!data.ok) {
        return new ErrorObject("Failed to fetch primebox", "xprime - primebox", data.status, "check if valid url", true, true);
    }
    data = await data.json();
    if (data.status !== "ok") {
        return new ErrorObject("primebox reported: " + data.details, "xprime - primebox", 404, undefined, true, false);
    }
    return handleSuccess(data, "en");
}

async function handleVolkswagen(media) {
    let data = await fetch(BACKEND_DOMAIN + `volkswagen?name=${media.title}&year=${media.releaseYear}`, {
        "headers": {
            Accept: "*/*",
            Referer: DOMAIN + "watch/" + media.tmdb,
            Origin: DOMAIN
    ...
    if (!data.ok) {
        return new ErrorObject("Failed to fetch volkswagen", "xprime - volkswagen", data.status, "check if valid url", true, true);
    }
    data = await data.json();
    if (data.status !== "ok") {
        return new ErrorObject("volkswagen reported: " + data.details, "xprime - volkswagen", 404, undefined, true, false);
    }
