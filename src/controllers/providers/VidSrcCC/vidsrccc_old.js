...
export async function getVidSrcCC(media) {
    return new ErrorObject(
        'This provider is mean :(',
        'VidSrcCC',
        500,
        "3 things: 1. They want a vrf token when requesting for server hashes, 2. you would also need 'v' attribute where something gets hashed and you also need the cf_clearance cookie to get the server hashes. After that, the existing logic would stay the same... (and if that isnt enough, they block IPs (403 = your ip is cooked))",
        true,
        false
    );
    ...
    vidsrcCCSources.forEach((source) => {
        source.subtitles.forEach((subtitle) => {
            subtitles.push({
                lang:
                    languageMap[subtitle.label.split(' ')[0]] || subtitle.lang,
                url: subtitle.file
            });
        });
    });
...
function getCloudflareClearance(media) {
    return new ErrorObject(
        'Cloudflare clearance cookie is missing',
        'VidSrcCC',
        500,
        "You need to obtain the cf_clearance cookie manually or implement a mechanism to bypass Cloudflare's protection.",
        true,
        true
    );
}
