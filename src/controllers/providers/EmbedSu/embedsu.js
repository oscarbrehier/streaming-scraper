...
export async function getEmbedsu(media) {
    ...
    try {
        ...
        if (!htmlSearch.ok) return new ErrorObject("Embed page structure has changed or is down", "EmbedSu", 500, "please check if the backend can access embed.su without being blocked", true, true);
        ...
        if (!hashEncode) return new ErrorObject("Embed does not have source hash", "EmbedSu", 500, undefined, true, true);

        const hashDecode = JSON.parse(await stringAtob(hashEncode));
        const mEncrypt = hashDecode.hash;
        if (!mEncrypt) return new ErrorObject("Something changed in the embedsu logic", "EmbedSu", 500, undefined, true, true);
        ...
        if (!secondDecode || secondDecode.length === 0) return new ErrorObject("Something changed in the embedsu logic", "EmbedSu", 500, undefined, true, true);
        ...
        for (const item of secondDecode) {
            ...
            if (!dataDirect.ok) return new ErrorObject("Embed.su API is down, or the hash path has changed", "EmbedSu", 500, undefined, true, true);
        ...
        return new ErrorObject("An error occurred" + e, "EmbedSu", 500, undefined, true, true);
    }
}
...
async function stringAtob(input) {
    ...
    if (str.length % 4 === 1) {
        throw new ErrorObject("The string is not correctly encoded", "EmbedSu", 500, "The string is not correctly encoded", true, true);
    }
