...
export async function getPrimewire(media) {
    if (!media.imdb) {
        return new ErrorObject("Primewire requires an IMDB ID", "Primewire", 500, "Please provide an IMDB ID from the tmdb.js file", true, true);
    }
    ...
    const files = embeddableServers
        ...
        .map((embedLink) => ({
            file: embedLink.file,
            type: embedLink.type,
            lang: "en", ...(embedLink.headers && {headers: embedLink.headers}),
        }));

    return {
        files: files.map((file) => {
            const result = {
                file: file.file, type: file.type, lang: file.lang,
            };
            if (file.headers) {
...
async function loadServers(link) {
    try {
        ...
        return new ErrorObject(`Error loading servers for link: ${link}`, "Primewire", 500, "Check the link or Primewire's server response.", true, true);
    }
}
